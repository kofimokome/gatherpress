...
class Venue
{
	...
	public function set_static_map(int $post_id, WP_Post $post, bool $update): void
	{
		if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) { // @codeCoverageIgnore
			return; // @codeCoverageIgnore
		}

		// Check if the user has setup a static map provider to generate static maps
		// todo: Logic here

		$blocks = parse_blocks($post->post_content);
		// Check if this post has the venue block.
		$blocks = array_filter($blocks, function ($block) {
			return $block['blockName'] === 'gatherpress/venue';
		});
		if (!empty($blocks)) {
			// Loop through all the venues and get their static maps
			$blocks = array_values($blocks);
		}

		$meta = $this->get_venue_meta($post_id, self::POST_TYPE);

		$latitude =$meta ['latitude'];
		$longitude =$meta ['longitude'];

		$map_zoom_level = $blocks[0]['attrs']['mapZoomLevel'] ?? 10;
		$map_height = $blocks[0]['attrs']['mapHeight'] ?? 300;

		$url = "https://maps.geoapify.com/v1/staticmap?style=osm-bright&height=$map_height&center=lonlat:$longitude,$latitude&zoom=$map_zoom_level&marker=lonlat:$longitude,$latitude;type:material;color:%23ff3421;icontype:awesome&apiKey=16cc43fd6ac44b8a826db4bfaa3c25c3";
		// check if this
		error_log($url);
		$results = wp_remote_get($url);
		$image_data = wp_remote_retrieve_body($results);
		$venue_meta = [];
		if (!empty($image_data)) {
			$filename = 'venue-map-' . $post_id . '.jpg';
			$upload = wp_upload_bits($filename, null, $image_data);

			if (!$upload['error']) {
				error_log("No error uploading files");
				// $upload['file'] contains the path on the server
				// $upload['url'] contains the public URL
				// You can now use this to update post meta or set featured image
				update_post_meta($post_id, '_gatherpress_static_map_url', $upload['url']);
				$venue_meta = (array)json_decode(get_post_meta($post_id, 'gatherpress_venue_information', true));
				$venue_meta['static_map_url'] = $upload['url'];

				update_post_meta($post_id, 'gatherpress_venue_information', json_encode($venue_meta));
			} else {
